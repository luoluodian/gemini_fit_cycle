# 任务清单 - 工程增强版 (Task Checklist - Engineered)

> **版本**: v3.0
> **最后更新**: 2026-01-27
> **适用项目**: Gemini Fit Cycle
> **优化重点**: 引入环境对齐、视觉审计、自动化契约及更严格的交付闭环
...
## 📋 Task 0: 上下文初始化与环境对齐 (v3.0 新增)

### 🎯 任务目标

确保 AI 助手在开始业务分析前，完全同步项目的技术约束、全局规范和环境配置，消除“幻觉”风险。

### 📥 输入文件

- `docs/init_docs/` 目录下的所有文件
- **`html/` 目录 (前端静态原型/样式参考)**
- `package.json` (如有)
- `.env.example` (如有)
...
1. **技术栈锁定**: 确认 Node.js, NestJS, Vue 3, Taro 等核心依赖的具体版本。
2. **规范索引**: 提取命名规范（大驼峰/小驼峰）、文件命名规则、Git 提交格式。

#### 步骤 2: 知识库预热

1. **建立索引**: 扫描现有文档，建立“关键词-文件”映射表。
2. **矛盾检测**: 检查 `init_docs` 中是否存在相互冲突的指令。
3. **原型索引**: 扫描 `html/` 文件夹，建立“页面文件名 -> 业务功能”的映射关系。
4. **样式提取**: 快速分析 `html/` 中的 CSS/JS 结构，识别通用的配色变量、类名规范（如 Tailwind 或 自定义 class）。

### 📤 输出文件

**文件名**: `docs/pj_docs/00_环境上下文简报.md`

**必须包含的章节**:

1. **技术环境快照**: 核心依赖版本列表。
2. **全局规范摘要**: 命名、目录、Git 规范速查。
3. **视觉资产清单**: 列出 `html/` 中已有的静态页面及其对应的业务模块。
4. **上下文加载确认**: 确认无冲突或已解决的冲突列表。

### ✅ 完成标准 (Definition of Done)

#### 必须满足 (Must Have)

- [ ] 所有 `init_docs` 文件已被读取并分析
- [ ] 明确识别出项目使用的所有主要技术栈及其版本
- [ ] 输出了包含规范摘要的简报文档
...
- `docs/pj_docs/00_环境上下文简报.md` (Task 0 输出)
...
- **[前端页面]**: 对应 `html/xxxx.html` 的 Vue 页面移植与逻辑绑定
- **[前端组件]**: 从 `html/` 中提取出的头部、底部、卡片等可复用 HTML 片段
- **[UI 资产]**: 明确引用的 `html` 静态资源路径
- **[前端服务]**: API 调用封装、状态管理
- **[公共逻辑]**: 工具函数、中间件、拦截器
...
- [ ] 任务粒度适中（2-8 小时可完成）
- [ ] 依赖关系清晰标注
- [ ] **UI 任务明确标注了对应的 `html` 参考文件 (例如: 参考 `html/login.html`)**

#### 质量检查 (Quality Check)

- [ ] **原子性**: 任务不可再拆分且能独立交付
- [ ] **可测试性**: 每个任务有明确的验收标准
- [ ] **UI 对齐**: (v3.0) 前端任务明确引用了设计规范或组件库
- [ ] **无遗漏**: 覆盖所有需求文档中的功能点
- [ ] **无冗余**: 没有重复的任务
- [ ] **可排期**: 基于依赖关系可生成开发计划
...
- `docs/pj_docs/00_环境上下文简报.md` (技术约束)
...
定义详细的数据库表结构和 API 接口契约，作为前后端并行开发的"唯一真理来源 (SSOT)"。
...
- 路径 (Path)
- 认证要求
- **(v3.0) 自动化标注**: 是否支持 Swagger/OpenAPI 生成
...
**看板格式**:
| 任务ID | 任务名称 | 优先级 | T5-分析 | T6-编码 | T7-测试 | T8-评审 | T9-交付 | 预估工时 | 状态 | 阻塞源 |
|--------|----------|--------|---------|---------|---------|---------|---------|----------|------| ------ |
...
- [ ] 检查是否涉及跨模块 Service
- [ ] 列出所有关联模块
- [ ] **UI 结构分析**: 打开对应的 `html` 文件，分析 DOM 结构，确定如何拆分为 Vue 组件。
...
在任务工作目录创建 `T5_分析文档_(原子任务名称).md`，包含：
...
- [ ] 创建/修改页面组件
- [ ] 创建/修改可复用组件
- [ ] **(v3.0) 视觉还原**: 对照设计规范实现样式 (Tailwind + Sass)
- [ ] 实现 API Service 调用
- [ ] 实现 Pinia Store (如需要)
- [ ] **静态迁移**: 将 `html/` 中的 HTML 结构移植到 Vue template 中。
- [ ] **样式重构**: 将 `html` 中的原生 CSS 转换为项目约定的 Tailwind/Sass 格式。
- [ ] **逻辑绑定**: 将静态写死的数据替换为 Pinia Store 状态或 API 响应数据。
- [ ] **交互实现**: 复刻 `html` 中演示的 JS 交互效果。
...
- [ ] 组件渲染性能分析
- [ ] **(v3.0) 视觉还原审计**: 检查 CSS 类名语义、响应式断点、Theme 变量使用是否规范
- [ ] 状态管理合理性检查

#### 8.3 视觉还原审计 (重点更新)

- [ ] **DOM 结构对比**: 检查 Vue 组件渲染后的结构是否意外破坏了原 `html` 的布局。
- [ ] **样式覆盖检查**: 确认 Tailwind 类名是否正确还原了原 `html` 的视觉样式（间距、颜色、字体）。
...
- [ ] 确认 T4 契约文档与代码完全一致
- [ ] 在 `docs/pj_docs/04.5_原子任务交付看板.md` 中将该任务状态标记为 ✅
...
1. **严禁跳级**: 禁止在 T5 (分析) 未完成的情况下直接开始 T6 (编码)。
2. **回溯规则**: 若在 T7 (测试) 发现架构设计缺陷，必须回溯至 T3 修改架构文档，并同步更新受影响的原子任务。
3. **技术债标记**: 若为了赶进度使用了 Mock 数据或临时方案，必须在看板“状态”栏标注 `⚠️ Tech Debt`。
4. **样式冲突**: 若 `html` 中的样式与 `coding_standard.md` 或全局样式冲突，以**全局样式/架构设计**为准，并需更新 `html` 对应的部分以保持同步（或在文档中备注差异）。
...
| v3.0 | 2026-01-27 | 工程增强版：增加 Task 0 上下文对齐、UI 审计、自动化契约支持及阻塞管理 |
