<template>
  ...
  >
    ...
    <view class="flex items-center justify-between mb-2">
      <!-- 左侧点击区域：直接进入详细配置 -->
      <view
        class="flex items-center flex-1 min-w-0 pr-2 active:opacity-60 transition-all"
        @click="$emit('edit')"
      >
      ...
      <view class="flex items-center space-x-3">
        <!-- 营养汇总 -->
        <view
          class="text-right active:opacity-60 transition-all"
          @click="$emit('edit')"
        >
          ...
          <view
            class="flex items-center space-x-1.5 text-[16rpx] text-gray-400 font-bold mt-0.5"
          >
        ...
        <view
          class="flex items-center h-8 bg-white border border-solid border-gray-100 rounded-full px-1 shadow-sm"
        >
          <!-- 1. 更多按钮 (⋮) -->
          <view
          ...
          <view
            v-if="foods.length > 0"
            class="w-[1rpx] h-4 bg-gray-100 mx-0.5"
          ></view>
...
const totalCalories = computed(() =>
  props.foods.reduce((s, f) => s + (f.calories || 0), 0),
);
const totalProtein = computed(() =>
  props.foods.reduce((s, f) => s + (f.protein || 0), 0),
);
const totalFat = computed(() =>
  props.foods.reduce((s, f) => s + (f.fat || 0), 0),
);
const totalCarbs = computed(() =>
  props.foods.reduce((s, f) => s + (f.carbs || 0), 0),
);
</script>
