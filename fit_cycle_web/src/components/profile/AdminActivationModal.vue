        ...
        <view class="space-y-4">
          ...
          <view>
            ...
            <input
              ...
              type="number"
              :cursor-spacing="50"
              class="px-4 border-[1rpx] border-solid border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent bg-white h-10 text-sm"
              placeholder="请输入生成数量"
            />
          </view>
      ...
      <view v-if="generatedCodes.length > 0" class="animate-fade-in">
        ...
        >
          <view class="flex items-center justify-between mb-3">
            <view class="flex items-center">
              ...
              <text class="text-sm font-semibold text-gray-700"
                >生成结果 ({{ generatedCodes.length }})</text
              >
            </view>
            <text
          ...
          <view class="max-h-40 overflow-y-auto space-y-2 pr-1">
            ...
            >
              <text class="font-mono text-sm text-gray-800 select-all">{{
                item.code
              }}</text>
              <text class="text-[18rpx] text-gray-400 font-bold"
                >{{ item.durationDays }}天</text
              >
            </view>
          </view>
        </GlassCard>
    ...
    <template #footer>
      <view class="flex space-x-3 p-4 pt-0 bg-gray-50">
        ...
        >
          <text
            v-if="submitting"
            class="nut-icon nut-icon-loading rotate-animation mr-2"
          ></text>
          {{ submitting ? "处理中..." : "立即生成" }}
        </view>
      </view>
...
const handleGenerate = async () => {
  ...
  if (!form.durationDays || form.durationDays <= 0)
    return showError("请输入有效时长");

  showLoading("正在生成...");
  submitting.value = true;
  try {
    const res: any = await httpRequest.post(
      "/admin/activation-codes/batch-generate",
      form,
    );
