...
class EmbeddedFoodSelector {
  ...
  createFoodItem(food) {
    ...
    item.className =
      "food-selector-item border border-gray-200 rounded-lg p-3 hover:border-emerald-300 hover:bg-emerald-50 transition-colors cursor-pointer";
    ...
    modal.innerHTML = `
            <div class="bg-white rounded-2xl p-6 w-full max-w-sm">
                <div class="text-center mb-4">
                    <h3 class="text-lg font-semibold text-gray-800">添加食物</h3>
                    <p class="text-sm text-gray-500">${food.name}</p>
                </div>
                <div class="mb-4 flex items-center">
                    <label class="text-sm font-medium text-gray-700 mr-3 whitespace-nowrap">数量</label>
                ...
                <div class="bg-gray-50 rounded-lg p-3 mb-4">
                    <div class="text-xs text-gray-500 mb-2">营养成分预览</div>
                    <div class="grid grid-cols-4 gap-2 text-center">
                        <div>
                            <div class="text-xs text-gray-500">碳水</div>
                            <div class="text-sm font-medium text-yellow-600" id="previewCarbs">${food.carbs}g</div>
                        </div>
                        <div>
                            <div class="text-xs text-gray-500">蛋白质</div>
                            <div class="text-sm font-medium text-red-600" id="previewProtein">${food.protein}g</div>
                        </div>
                        <div>
                            <div class="text-xs text-gray-500">脂肪</div>
                            <div class="text-sm font-medium text-blue-600" id="previewFat">${food.fat}g</div>
                        </div>
                        <div>
                            <div class="text-xs text-gray-500">能量</div>
                            <div class="text-sm font-medium text-emerald-600" id="previewCalories">${food.calories} kcal</div>
                        </div>
                    </div>
                </div>
                <div class="flex space-x-3">
                    ...
                    <button onclick="confirmAddFoodFromEmbedded('${food.name}', ${food.calories}, ${food.protein}, ${food.fat}, ${food.carbs})" class="flex-1 bg-emerald-600 text-white py-2 px-4 rounded-lg font-medium hover:bg-emerald-700 transition-colors">
                        确认添加
                    </button>
                </div>
            </div>
  ...
  updatePreview(food) {
    const quantity =
      parseFloat(document.getElementById("foodQuantity").value) || 0;
