...
### 1.3 认证与安全 (Auth & Security)
- `modules/auth/jwt.guard.ts`: JWT 访问令牌守卫
- `modules/auth/jwt.strategy.ts`: JWT 访问令牌验证策略
- `modules/auth/jwt-refresh.guard.ts`: JWT 刷新令牌守卫
- `modules/auth/jwt-refresh.strategy.ts`: JWT 刷新令牌验证策略
...
- `dtos/auth-response.dto.ts`: 登录成功响应 (含 Token 与用户基础信息)
- `dtos/user.dto.ts`: 用户更新、微信授权、健康档案 DTO
- `dtos/food-item.dto.ts`: 食材 CRUD 及收藏 DTO
- `dtos/create-diet-plan.dto.ts`: 计划创建 DTO
- `dtos/update-diet-plan.dto.ts`: 计划更新 DTO
- `dtos/create-plan-day.dto.ts`: 计划天数配置创建
- `dtos/create-plan-meal.dto.ts`: 计划餐次配置创建
- `dtos/create-plan-meal-item.dto.ts`: 计划餐次食材配置创建
- `dtos/update-plan-day.dto.ts`: 计划天数配置部分更新
- `dtos/update-plan-day-full.dto.ts`: 计划天数配置全量更新
- `dtos/update-plan-meal.dto.ts`: 计划餐次配置更新
- `dtos/update-plan-meal-item.dto.ts`: 计划餐次食材配置更新
- `dtos/init-plan-days.dto.ts`: 计划天数批量初始化
- `dtos/save-plan-templates.dto.ts`: 计划模板保存
- `dtos/create-meal-log.dto.ts`: 餐次记录创建
- `dtos/update-meal-log.dto.ts`: 餐次记录更新
- `dtos/sync-meal.dto.ts`: 餐次打卡数据同步
- `dtos/import-plan.dto.ts`: 计划导入 (含分享码验证)
- `dtos/dict.dto.ts`: 字典查询与管理
...
- `common/logger/logger.module.ts`: 日志模块
- `common/logger/trace.middleware.ts`: TraceId 追踪中间件
- `common/logger/winston.config.ts`: Winston 配置
- `common/transformers/user.transformer.ts`: 用户嵌套结构转换器
- `common/transformers/dict.transformer.ts`: 字典数据响应转换器
- `common/utils/trace.util.ts`: 链路追踪工具
- `common/utils/pagination.helper.ts`: 分页查询辅助工具
- `common/utils/sort.helper.ts`: 排序参数处理辅助工具
...
### 2.2 核心组件 (Components)
- `components/home/*`: 首页相关 (DailyGoalsOverview, DateNavigation, HomeMealCard)
- `components/food/*`: 食材相关 (FoodPicker, FoodSelectorModal, FoodDetailModal, FoodItemCard)
- `components/plan/*`: 计划相关 (PlanCard, ImportPlanModal, CreateOptionsModal, EmptyPlanState, NewPlanModal, PlanTabs)
- `components/plan-creator/*`: 创建流程步骤组件
- `components/daily-list/*`: 列表相关组件 (DayCard, StatsCard, FilterButtons, BatchModal)
- `components/common/*`: 通用原子组件 (GlassCard, BaseButton, BaseModal, BaseNavBar, BaseScrollView, BaseTabs, MealCard, MenuItem, PageLayout, SearchBar)
- `custom-tab-bar/*`: 自定义 TabBar 组件

### 2.3 业务逻辑钩子 (Composables)
- `composables/useNutritionStats.ts`: 营养数据统计、BMR/TDEE 计算钩子
...
- `services/http/types.ts`: HTTP 请求相关类型定义
- `services/modules/*`: 按业务模块划分的 API 调用 (auth.ts, food.ts, plan.ts, record.ts, user.ts)

### 2.6 导航与路由 (Router)
- `router/index.ts`: 路由配置与钩子
- `router/hooks.ts`: 导航守卫 Hooks
- `router/types.ts`: 路由相关类型
- `router/utils.ts`: 路由工具函数

### 2.7 常量与类型 (Constants & Types)
- `constants/api-config.ts`: API 端点与白名单配置
- `constants/error-codes.ts`: 业务错误码定义
- `constants/food-categories.ts`: 食材分类常量
- `constants/routes.ts`: 页面路由路径常量
- `types/plan.ts`: 饮食计划相关 TS 类型定义
- `types/food-constants.ts`: 食材相关 TS 类型定义
...
- `case/test_integration_i4_1.py`: **I-4.1 接口集成测试用例** (侧重后端 API 连通性)
- `case/test_record_module.py`: 饮食记录模块自动化测试
- `case/test_real_click_flow.py`: 真实交互点击流测试
