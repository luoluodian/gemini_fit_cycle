...
- `user_id`: FK -> users.id
- `date`: Date, Index
- `target_calories`: Int (当天目标能量快照)
- `target_protein`: Decimal (当天目标蛋白质快照)
- `target_fat`: Decimal (当天目标脂肪快照)
- `target_carbs`: Decimal (当天目标碳水快照)
- `plan_id`: FK -> diet_plans.id (Nullable)

**`meal_logs`**
- `id`: PK
- `user_id`: BigInt (冗余鉴权字段)
- `record_id`: FK -> daily_records.id
- `meal_type`: Enum('breakfast', 'lunch', 'dinner', 'snacks')
- `food_id`: FK -> food_items.id
- `food_name`: Varchar(100) (快照)
- `custom_name`: Varchar(255) (可选)
- `quantity`: Decimal
- `unit`: Varchar(20)
- `base_count`: Int (营养计算基准, 默认100)
- `calories`: Int (当前摄入热量)
- `protein`: Decimal (当前摄入蛋白质)
- `fat`: Decimal (当前摄入脂肪)
- `carbs`: Decimal (当前摄入碳水)
- `base_calories`: Int (100g/单位基准热量快照)
- `base_protein`: Decimal (基准蛋白质快照)
- `base_fat`: Decimal (基准脂肪快照)
- `base_carbs`: Decimal (基准碳水快照)
- `is_planned`: Boolean (是否源自计划)
- `is_recorded`: Boolean (是否已确认打卡)
...
### 2.1 认证 (Auth)
*   **POST /auth/login**
    *   Body: `{ code: string, rawData?: string }`
    *   Resp: `{ accessToken: string, refreshToken: string, user: UserProfile }`
...
*   **POST /records/meal**
    *   Body: `{ date, mealType, foodId, quantity, isPlanned? }`
    *   Resp: `MealLog` (含计算后的热量)
*   **PUT /records/meal/:id**
    *   Body: `{ quantity?, isRecorded? }`
    *   Resp: `MealLog`
*   **POST /records/meal/sync**
    *   Body: `{ date, mealType }`
    *   Desc: 按激活计划同步指定餐次的食物到当日记录
    *   Resp: `MealLog[]`
