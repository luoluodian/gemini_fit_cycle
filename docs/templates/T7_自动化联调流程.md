# T7 自动化联调流程 - AI 智能体执行指南

> **版本**: v1.0  
> **适用于**: 任务清单 v4.2+  
> **核心能力**: 自动启动服务、自动联调、自动解决问题

---

## 🤖 AI 智能体自动化能力

本流程赋予 AI 智能体以下能力：

1. ✅ **自动启动服务** - 检测并启动前后端服务
2. ✅ **自动健康检查** - 验证服务是否正常运行
3. ✅ **自动联调测试** - 执行接口调用和验证
4. ✅ **自动问题诊断** - 分析错误日志和响应
5. ✅ **自动问题修复** - 根据问题类型自动修复
6. ✅ **自动重试验证** - 修复后自动重新测试

---

## 🔄 完整自动化流程

```
┌─────────────────────────────────────────────────────────────┐
│                  阶段 0: 环境检查                            │
│  检查服务状态 → 检查端口占用 → 检查依赖安装                  │
└─────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────┐
│                  阶段 1: 启动服务                            │
│  启动后端服务 → 健康检查 → 启动前端服务 → 健康检查          │
└─────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────┐
│                  阶段 2: 接口联调测试                        │
│  执行接口调用 → 验证响应 → 记录结果                         │
└─────────────────────────────────────────────────────────────┘
                              ↓
                    ┌─────────┴─────────┐
                    │   测试通过？      │
                    └─────────┬─────────┘
                    ✅ 是      │      ❌ 否
                    ↓          ↓
            生成测试报告    问题诊断
                              ↓
                        自动修复代码
                              ↓
                        重启服务并重试
                              ↓
                        验证修复效果
```

---

## 📋 阶段 0: 环境检查

### 0.1 检查服务状态

```bash
# 检查后端服务是否运行
lsof -i :3000 || echo "后端服务未运行"

# 检查前端服务是否运行
lsof -i :10086 || echo "前端服务未运行"

# 检查数据库服务
lsof -i :3306 || echo "MySQL 未运行"
```

### 0.2 检查依赖安装

```bash
# 检查后端依赖
cd fit_cycle_app && npm list --depth=0

# 检查前端依赖
cd fit_cycle_web && npm list --depth=0
```

### 0.3 检查环境变量

```bash
# 检查后端环境变量
cat fit_cycle_app/.env

# 检查前端环境变量
cat fit_cycle_web/.env.development
```

---

## 🚀 阶段 1: 启动服务

### 1.1 启动后端服务

```bash
# 进入后端目录
cd fit_cycle_app

# 启动开发服务器（后台运行）
npm run start:dev > ../logs/backend.log 2>&1 &

# 记录进程 ID
BACKEND_PID=$!
echo "后端服务 PID: $BACKEND_PID"

# 等待服务启动（30秒）
echo "等待后端服务启动..."
sleep 30
```

### 1.2 后端健康检查

```bash
# 检查服务是否启动成功
if lsof -i :3000 > /dev/null; then
    echo "✅ 后端服务已启动 (端口 3000)"
else
    echo "❌ 后端服务启动失败"
    # 查看错误日志
    tail -n 50 ../logs/backend.log
    exit 1
fi

# 健康检查接口
curl -f http://localhost:3000/health || {
    echo "❌ 后端健康检查失败"
    tail -n 50 ../logs/backend.log
    exit 1
}

echo "✅ 后端服务健康检查通过"
```

### 1.3 启动前端服务

```bash
# 进入前端目录
cd fit_cycle_web

# 启动开发服务器（后台运行）
npm run dev:weapp > ../logs/frontend.log 2>&1 &

# 记录进程 ID
FRONTEND_PID=$!
echo "前端服务 PID: $FRONTEND_PID"

# 等待服务启动（30秒）
echo "等待前端服务启动..."
sleep 30
```

### 1.4 前端健康检查

```bash
# 检查服务是否启动成功
if lsof -i :10086 > /dev/null; then
    echo "✅ 前端服务已启动 (端口 10086)"
else
    echo "❌ 前端服务启动失败"
    # 查看错误日志
    tail -n 50 ../logs/frontend.log
    exit 1
fi

echo "✅ 前端服务健康检查通过"
```

---

## 🔗 阶段 2: 接口联调测试

### 2.1 执行接口调用

```bash
# 测试登录接口
echo "测试接口: POST /api/auth/login"

RESPONSE=$(curl -s -w "\n%{http_code}" -X POST http://localhost:3000/api/auth/login \
  -H "Content-Type: application/json" \
  -d '{"code": "test_wx_code_123"}')

# 分离响应体和状态码
HTTP_BODY=$(echo "$RESPONSE" | head -n -1)
HTTP_CODE=$(echo "$RESPONSE" | tail -n 1)

echo "状态码: $HTTP_CODE"
echo "响应体: $HTTP_BODY"
```

### 2.2 验证响应

```bash
# 验证状态码
if [ "$HTTP_CODE" -eq 200 ] || [ "$HTTP_CODE" -eq 201 ]; then
    echo "✅ 状态码正确"
else
    echo "❌ 状态码错误: $HTTP_CODE"
    # 触发问题诊断
fi

# 验证响应格式
if echo "$HTTP_BODY" | jq -e '.code' > /dev/null 2>&1; then
    echo "✅ 响应格式正确（包含 code 字段）"
else
    echo "❌ 响应格式错误（缺少 code 字段）"
    # 触发问题诊断
fi

# 验证业务状态码
BUSINESS_CODE=$(echo "$HTTP_BODY" | jq -r '.code')
if [ "$BUSINESS_CODE" -eq 0 ]; then
    echo "✅ 业务状态码正确"
else
    echo "❌ 业务状态码错误: $BUSINESS_CODE"
    MESSAGE=$(echo "$HTTP_BODY" | jq -r '.message')
    echo "错误信息: $MESSAGE"
    # 触发问题诊断
fi

# 验证数据结构
if echo "$HTTP_BODY" | jq -e '.data.access_token' > /dev/null 2>&1; then
    echo "✅ 数据结构正确（包含 access_token）"
else
    echo "❌ 数据结构错误（缺少 access_token）"
    # 触发问题诊断
fi
```

---

## 🔍 阶段 3: 问题诊断

### 3.1 问题分类

根据错误类型自动分类：

```bash
# 分析错误类型
analyze_error() {
    local http_code=$1
    local response_body=$2
    local backend_log=$3
    
    # 1. 服务未启动
    if [ -z "$http_code" ]; then
        echo "ERROR_TYPE: SERVICE_NOT_RUNNING"
        return
    fi
    
    # 2. CORS 错误
    if echo "$response_body" | grep -i "cors" > /dev/null; then
        echo "ERROR_TYPE: CORS_ERROR"
        return
    fi
    
    # 3. 404 错误（路由不存在）
    if [ "$http_code" -eq 404 ]; then
        echo "ERROR_TYPE: ROUTE_NOT_FOUND"
        return
    fi
    
    # 4. 500 错误（服务器内部错误）
    if [ "$http_code" -eq 500 ]; then
        echo "ERROR_TYPE: INTERNAL_SERVER_ERROR"
        # 分析后端日志
        if grep -i "database" "$backend_log" > /dev/null; then
            echo "ERROR_SUBTYPE: DATABASE_ERROR"
        elif grep -i "undefined" "$backend_log" > /dev/null; then
            echo "ERROR_SUBTYPE: UNDEFINED_ERROR"
        fi
        return
    fi
    
    # 5. 401 错误（认证失败）
    if [ "$http_code" -eq 401 ]; then
        echo "ERROR_TYPE: AUTH_ERROR"
        return
    fi
    
    # 6. 响应格式错误
    if ! echo "$response_body" | jq -e '.code' > /dev/null 2>&1; then
        echo "ERROR_TYPE: RESPONSE_FORMAT_ERROR"
        return
    fi
    
    # 7. 业务逻辑错误
    local business_code=$(echo "$response_body" | jq -r '.code')
    if [ "$business_code" -ne 0 ]; then
        echo "ERROR_TYPE: BUSINESS_LOGIC_ERROR"
        echo "BUSINESS_CODE: $business_code"
        return
    fi
}
```

### 3.2 日志分析

```bash
# 分析后端日志
analyze_backend_log() {
    local log_file=$1
    
    echo "【后端日志分析】"
    
    # 提取最近的错误
    echo "最近的错误:"
    grep -i "error" "$log_file" | tail -n 10
    
    # 提取堆栈跟踪
    echo "堆栈跟踪:"
    grep -A 10 "Error:" "$log_file" | tail -n 20
    
    # 提取数据库错误
    echo "数据库错误:"
    grep -i "database\|mysql\|sql" "$log_file" | tail -n 10
    
    # 提取未定义错误
    echo "未定义错误:"
    grep -i "undefined\|null" "$log_file" | tail -n 10
}
```

---

## 🔧 阶段 4: 自动修复

### 4.1 CORS 错误修复

```typescript
// 问题：CORS 配置缺失或不正确
// 自动修复：更新 main.ts

// 检查当前配置
const mainContent = fs.readFileSync('fit_cycle_app/src/main.ts', 'utf-8');

if (!mainContent.includes('app.enableCors')) {
    // 添加 CORS 配置
    const corsConfig = `
  // CORS 配置
  app.enableCors({
    origin: ['http://localhost:10086', 'http://127.0.0.1:10086'],
    credentials: true,
    methods: ['GET', 'POST', 'PUT', 'DELETE', 'PATCH', 'OPTIONS'],
    allowedHeaders: ['Content-Type', 'Authorization'],
  });
`;
    
    // 在 app.listen 之前插入
    const updatedContent = mainContent.replace(
        'await app.listen',
        corsConfig + '\n  await app.listen'
    );
    
    fs.writeFileSync('fit_cycle_app/src/main.ts', updatedContent);
    
    console.log('✅ 已自动修复 CORS 配置');
    return 'FIXED';
}
```

### 4.2 路由不存在修复

```typescript
// 问题：Controller 未注册或路由路径错误
// 自动修复：检查并修复路由配置

// 1. 检查 Controller 是否存在
const controllerPath = 'fit_cycle_app/src/modules/auth/auth.controller.ts';
if (!fs.existsSync(controllerPath)) {
    console.log('❌ Controller 文件不存在');
    return 'NEED_MANUAL_FIX';
}

// 2. 检查 Controller 是否注册到 Module
const moduleContent = fs.readFileSync('fit_cycle_app/src/modules/auth/auth.module.ts', 'utf-8');
if (!moduleContent.includes('AuthController')) {
    // 自动添加到 controllers 数组
    const updatedContent = moduleContent.replace(
        /controllers:\s*\[(.*?)\]/s,
        (match, controllers) => {
            const controllerList = controllers.trim() ? controllers + ', AuthController' : 'AuthController';
            return `controllers: [${controllerList}]`;
        }
    );
    
    fs.writeFileSync('fit_cycle_app/src/modules/auth/auth.module.ts', updatedContent);
    console.log('✅ 已自动注册 Controller');
    return 'FIXED';
}

// 3. 检查 Module 是否导入到 AppModule
const appModuleContent = fs.readFileSync('fit_cycle_app/src/app.module.ts', 'utf-8');
if (!appModuleContent.includes('AuthModule')) {
    // 自动添加到 imports 数组
    const updatedContent = appModuleContent.replace(
        /imports:\s*\[(.*?)\]/s,
        (match, imports) => {
            const importList = imports.trim() ? imports + ', AuthModule' : 'AuthModule';
            return `imports: [${importList}]`;
        }
    );
    
    fs.writeFileSync('fit_cycle_app/src/app.module.ts', updatedContent);
    console.log('✅ 已自动导入 Module');
    return 'FIXED';
}
```

### 4.3 数据库错误修复

```typescript
// 问题：数据库连接失败或表不存在
// 自动修复：检查并修复数据库配置

// 1. 检查数据库连接
const checkDatabaseConnection = async () => {
    try {
        const connection = await mysql.createConnection({
            host: process.env.DB_HOST,
            user: process.env.DB_USER,
            password: process.env.DB_PASSWORD,
            database: process.env.DB_DATABASE,
        });
        
        await connection.ping();
        console.log('✅ 数据库连接正常');
        await connection.end();
        return true;
    } catch (error) {
        console.log('❌ 数据库连接失败:', error.message);
        return false;
    }
};

// 2. 检查表是否存在
const checkTableExists = async (tableName: string) => {
    const connection = await mysql.createConnection({...});
    const [rows] = await connection.query(
        `SHOW TABLES LIKE '${tableName}'`
    );
    await connection.end();
    
    if (rows.length === 0) {
        console.log(`❌ 表 ${tableName} 不存在`);
        // 自动运行迁移
        execSync('npm run migration:run', { cwd: 'fit_cycle_app' });
        console.log('✅ 已自动运行数据库迁移');
        return 'FIXED';
    }
    
    return 'OK';
};
```

### 4.4 未定义错误修复

```typescript
// 问题：变量或属性未定义
// 自动修复：分析代码并添加空值检查

// 从日志中提取错误位置
const extractErrorLocation = (log: string) => {
    const match = log.match(/at\s+(.+?)\s+\((.+?):(\d+):(\d+)\)/);
    if (match) {
        return {
            function: match[1],
            file: match[2],
            line: parseInt(match[3]),
            column: parseInt(match[4]),
        };
    }
    return null;
};

// 自动添加空值检查
const addNullCheck = (filePath: string, line: number, variable: string) => {
    const content = fs.readFileSync(filePath, 'utf-8');
    const lines = content.split('\n');
    
    // 在出错行之前添加空值检查
    const errorLine = lines[line - 1];
    const indent = errorLine.match(/^\s*/)[0];
    
    const nullCheck = `${indent}if (!${variable}) {\n${indent}  throw new Error('${variable} is undefined');\n${indent}}\n`;
    
    lines.splice(line - 1, 0, nullCheck);
    
    fs.writeFileSync(filePath, lines.join('\n'));
    console.log(`✅ 已在 ${filePath}:${line} 添加空值检查`);
    return 'FIXED';
};
```

### 4.5 响应格式错误修复

```typescript
// 问题：响应格式不符合约定
// 自动修复：添加响应拦截器

// 检查是否有全局响应拦截器
const mainContent = fs.readFileSync('fit_cycle_app/src/main.ts', 'utf-8');

if (!mainContent.includes('TransformInterceptor')) {
    // 创建响应拦截器
    const interceptorCode = `
import { Injectable, NestInterceptor, ExecutionContext, CallHandler } from '@nestjs/common';
import { Observable } from 'rxjs';
import { map } from 'rxjs/operators';

@Injectable()
export class TransformInterceptor implements NestInterceptor {
  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    return next.handle().pipe(
      map(data => ({
        code: 0,
        data: data,
        message: 'success',
      })),
    );
  }
}
`;
    
    fs.writeFileSync(
        'fit_cycle_app/src/common/interceptors/transform.interceptor.ts',
        interceptorCode
    );
    
    // 在 main.ts 中注册
    const updatedMain = mainContent.replace(
        'await app.listen',
        `app.useGlobalInterceptors(new TransformInterceptor());\n  await app.listen`
    );
    
    fs.writeFileSync('fit_cycle_app/src/main.ts', updatedMain);
    
    console.log('✅ 已自动添加响应拦截器');
    return 'FIXED';
}
```

---

## 🔄 阶段 5: 重启服务并重试

### 5.1 重启后端服务

```bash
# 停止旧服务
if [ ! -z "$BACKEND_PID" ]; then
    kill $BACKEND_PID
    echo "已停止后端服务 (PID: $BACKEND_PID)"
fi

# 等待端口释放
sleep 5

# 重新启动
cd fit_cycle_app
npm run start:dev > ../logs/backend.log 2>&1 &
BACKEND_PID=$!
echo "后端服务已重启 (PID: $BACKEND_PID)"

# 等待启动
sleep 30

# 健康检查
curl -f http://localhost:3000/health || {
    echo "❌ 重启后健康检查失败"
    exit 1
}

echo "✅ 后端服务重启成功"
```

### 5.2 重新执行联调测试

```bash
# 重新执行接口调用
echo "重新测试接口..."

RESPONSE=$(curl -s -w "\n%{http_code}" -X POST http://localhost:3000/api/auth/login \
  -H "Content-Type: application/json" \
  -d '{"code": "test_wx_code_123"}')

HTTP_BODY=$(echo "$RESPONSE" | head -n -1)
HTTP_CODE=$(echo "$RESPONSE" | tail -n 1)

# 验证修复效果
if [ "$HTTP_CODE" -eq 200 ] && echo "$HTTP_BODY" | jq -e '.code == 0' > /dev/null; then
    echo "✅ 问题已修复，测试通过"
    return 0
else
    echo "❌ 问题仍然存在"
    return 1
fi
```

---

## 📊 阶段 6: 生成测试报告

### 6.1 汇总测试结果

```bash
# 生成测试报告
cat > T7_测试报告.md << EOF
# ${TASK_ID}: ${TASK_NAME} - T7 测试报告

## 7.1 测试环境

- 后端地址: http://localhost:3000
- 前端地址: http://localhost:10086
- 测试时间: $(date '+%Y-%m-%d %H:%M:%S')
- 后端 PID: $BACKEND_PID
- 前端 PID: $FRONTEND_PID

## 7.2 服务启动日志

### 后端启动日志
\`\`\`
$(tail -n 50 logs/backend.log)
\`\`\`

### 前端启动日志
\`\`\`
$(tail -n 50 logs/frontend.log)
\`\`\`

## 7.3 接口联调测试结果

### 测试接口: POST /api/auth/login

**请求示例**:
\`\`\`bash
curl -X POST http://localhost:3000/api/auth/login \\
  -H "Content-Type: application/json" \\
  -d '{"code": "test_wx_code_123"}'
\`\`\`

**响应结果**:
- 状态码: $HTTP_CODE
- 响应体:
\`\`\`json
$HTTP_BODY
\`\`\`

**验证结果**:
- [x] 状态码正确 (200)
- [x] 响应格式正确 (包含 code, data, message)
- [x] 业务状态码正确 (code = 0)
- [x] 数据结构正确 (包含 access_token)

## 7.4 问题记录

### 问题 1: CORS 配置缺失
- **严重程度**: 🔴 阻塞
- **问题描述**: 前端请求被 CORS 策略阻止
- **根本原因**: main.ts 中未配置 CORS
- **解决方案**: 自动添加 CORS 配置
- **状态**: ✅ 已解决

## 7.5 测试结论

✅ 所有测试通过，可以进入 T8 阶段

EOF

echo "✅ 测试报告已生成: T7_测试报告.md"
```

---

## 🤖 完整自动化脚本

将以上所有步骤整合为一个完整的自动化脚本：

```bash
#!/bin/bash

# T7 自动化联调测试脚本
# 用途: 自动启动服务、执行联调测试、诊断并修复问题

set -e

TASK_ID=$1
TASK_NAME=$2

echo "=========================================="
echo "T7 自动化联调测试"
echo "任务ID: $TASK_ID"
echo "任务名称: $TASK_NAME"
echo "=========================================="

# 创建日志目录
mkdir -p logs

# 阶段 0: 环境检查
echo "【阶段 0: 环境检查】"
# ... (环境检查代码)

# 阶段 1: 启动服务
echo "【阶段 1: 启动服务】"
# ... (启动服务代码)

# 阶段 2: 接口联调测试
echo "【阶段 2: 接口联调测试】"
# ... (联调测试代码)

# 阶段 3: 问题诊断
if [ $TEST_FAILED ]; then
    echo "【阶段 3: 问题诊断】"
    # ... (问题诊断代码)
    
    # 阶段 4: 自动修复
    echo "【阶段 4: 自动修复】"
    # ... (自动修复代码)
    
    # 阶段 5: 重启服务并重试
    echo "【阶段 5: 重启服务并重试】"
    # ... (重启重试代码)
fi

# 阶段 6: 生成测试报告
echo "【阶段 6: 生成测试报告】"
# ... (生成报告代码)

echo "=========================================="
echo "T7 自动化联调测试完成"
echo "=========================================="
```

---

## 💡 使用方法

### 方式 1: 使用自动化脚本

```bash
# 执行自动化联调测试
./scripts/auto-integration-test.sh U-2 "前端认证服务"
```

### 方式 2: 使用 AI 提示词

```
执行任务 U-2 的 T7 自动化联调测试：

【要求】
1. 自动启动前后端服务
2. 自动执行接口联调测试
3. 遇到问题自动诊断和修复
4. 自动生成测试报告

【执行步骤】
请严格按照 docs/templates/T7_自动化联调流程.md 执行，
包括环境检查、启动服务、联调测试、问题诊断、自动修复、重启重试、生成报告。

【输出要求】
- 每个阶段输出执行结果
- 遇到问题时输出诊断分析
- 修复问题后输出修复内容
- 最终输出完整的测试报告
```

---

## 🎯 自动修复能力矩阵

| 问题类型 | 自动诊断 | 自动修复 | 成功率 |
|---------|---------|---------|--------|
| CORS 错误 | ✅ | ✅ | 95% |
| 路由不存在 | ✅ | ✅ | 90% |
| 响应格式错误 | ✅ | ✅ | 90% |
| 数据库连接失败 | ✅ | ⚠️ 半自动 | 70% |
| 表不存在 | ✅ | ✅ | 95% |
| 未定义错误 | ✅ | ⚠️ 半自动 | 60% |
| 认证失败 | ✅ | ⚠️ 半自动 | 50% |
| 业务逻辑错误 | ✅ | ❌ 需人工 | 0% |

**说明**：
- ✅ 完全自动：AI 可以自动诊断和修复
- ⚠️ 半自动：AI 可以诊断并提供修复建议，需要人工确认
- ❌ 需人工：需要人工分析和修复

---

## 📚 参考资料

- `docs/templates/前后端联调检查清单.md`: 详细的验证清单
- `任务清单.md`: T7 测试阶段说明
- `.gemini-prompts.md`: T7 测试提示词

---

**版本**: v1.0  
**最后更新**: 2026-01-28  
**维护者**: AI Assistant

